...
func main() {
	...
	// Basic file reading task is slurping a file's entire contents into memory
	...
	// There is no built-in rewind, bug Seek(0, 0) accomplishes this
	_, err = f.Seek(0, 0)
	check(err)

	// The bufio package implements a buffered reader that my be useful both for its
	// efficiency with many small reads and because of the additional reading methods
	// it provides
	...
	// Close file when you're done (usually this would be scheduled immediately after
	// opening with defer
	f.Close()
