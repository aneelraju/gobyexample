...
// For example, the GIT revision control system uses SHA1's
// extensively to identify versioned files and directories
package main
...
func main() {
	...
	// The pattern for generating a has is sha1.New(), sha1.Write(bytes) then
	// sha1.Sum([]byte{})
	h := sha1.New()
	...
	// Note if you need cryptographically secure hashes, you should carefully research
	// hash strength
}
